...
export class ApplyLoanCompComponent implements OnInit {
  ...
  constructor(
    ...
    private applyLoanService: LoanApplyService
  ) {
  ...
  ngOnInit(): void {
    this.basicFormGroup = this._formBuilder.group({
      ...
      rateOfInterest: new FormControl({ value: '', disabled: true }, [
        Validators.required
      ]),
      ...
      eLoanFormGroup: new FormGroup({
        ...
        fatherExp: new FormControl('', [
          Validators.required,
          Validators.pattern('^[0-9]*$')
        ]),
        fatherExpwithCompany: new FormControl('', [
          Validators.required,
          Validators.pattern('^[0-9]*$')
        ]),
        ...
        eannualIncome: new FormControl('', [
          Validators.required,
          Validators.pattern('^[0-9]*$')
        ])
      }),
      phLoanFormGroup: this._formBuilder.group({
        phannualIncome: new FormControl('', [
          Validators.required,
          Validators.pattern('^[0-9]*$')
        ]),
        ...
        expWithCompany: new FormControl('', [
          Validators.required,
          Validators.pattern('^[0-9]*$')
        ])
      })
  ...
  onChange(event) {
    if (event.value === 'Education Loan') {
      ...
      this.basicFormGroup
        .get('eLoanFormGroup.fatherExpwithCompany')
        .setValue('');
  ...
  applyLoan() {
    ...
    if (loanType === 'Education Loan' && obj2.valid) this.customSubmit();
  ...
  clearValidation() {
    if (this.basicFormGroup.get('loanType').value === 'Education Loan') {
      this.basicFormGroup
        .get('phLoanFormGroup.phannualIncome')
        .clearValidators();
      ...
      this.basicFormGroup
        .get('phLoanFormGroup.expWithCompany')
        .clearValidators();
    } else {
      ...
      this.basicFormGroup
        .get('eLoanFormGroup.fatherOccupation')
        .clearValidators();
